...
def plot_season_profiles(case_name, season, load_df, wind_df, hydro_series, rep_days, save_path=None):
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig(f"qubo_allcases_{season}_k_4.pdf", format="pdf")
    plt.show()
...
def plot_tsne(case_name, season, full_daily_features, rep_days,
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{season}_tsne_k_4.pdf", format="pdf")
    plt.show()
...
def main():
        ...
        for season in seasons:
            ...
            for month in sorted(load_season['Month'].unique()):
                ...
                for i in range(2):
                    ...
                    selected = get_rep_days_qubo(daily_features, k=4, penalty=1e4, reps=1)
    ...
    results_df.to_excel("annual_cost_deviation_summary_k_4.xlsx", index=False)
    ...
    plt.tight_layout()
    plt.savefig("qubo_deviation_by_case_season_k_4_even_odd.pdf")
    plt.show()
    # Grouped bar
    ...
    plt.tight_layout()
    plt.savefig("full_vs_qubo_cost_by_case_k_4_even_odd.pdf")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("qubo_deviation_lineplot_k_4_even_odd.pdf", format='pdf', bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("average_annual_deviation_by_case_k_4_even_odd.pdf")
    plt.show()
    ...
    plt.xlabel("Season")
    plt.tight_layout()
    plt.savefig("deviation_heatmap_k_4_even_odd.pdf", format="pdf")
    plt.show()
